...
const Board = () => {
  ...
  const handleOnDragEnd = (result: DropResult) => {
    ...
    // check if user drag card outside the board if so don't do anything
    if (!destination) return;

    // handle a culumn drag
    if (type === "column") {
    ...
    if (source.index === destination.index && startCol === finishCol) return; // if drag and drop in the exact same position do nothing
